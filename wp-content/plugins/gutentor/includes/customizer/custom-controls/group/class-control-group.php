...
if ( class_exists( 'WP_Customize_Control' ) && !class_exists( 'Gutentor_Custom_Control_Group' )):
	/**
	 ...
	 *
	 */
	class Gutentor_Custom_Control_Group extends WP_Customize_Control {
		...
		public function render_content() {
			...
			?>
            <ul class="gutentor-group-field-control-wrap">
				<?php $this->get_fields(); ?>
            </ul>
            <input type="hidden" <?php $this->link(); ?> class="gutentor-group-collection" value="<?php echo esc_attr( $values ); ?>"/>
			<?php
		}
		...
		private function get_fields() {
            ...
            ?>
            <li class="gutentor-group-field-control">
					...
					foreach ( $fields as $key => $field ) {
                        ...
                        <div class="single-field type-<?php echo esc_attr( $field['type'] ) . ' ' . $class; ?>">
							...
							switch ( $field['type'] ) {
								...
								case 'color':
									echo '<div class="customize-control-alpha-color" data-color-single-name="' . esc_attr( $key ) . '"><input class="alpha-color-control" data-default="' . esc_attr( $default ) . '" data-single-name="' . esc_attr( $key ) . '" type="text" value="' . esc_attr( $new_value ) . '"/></div>';
									break;
								...
								case 'image':
									...
									$gutentor_display_none = '';
									if ( empty( $new_value ) ){
										$gutentor_display_none = ' style="display:none;" ';
									}
									...
									?>
                                    <span class="img-preview-wrap" <?php echo  $gutentor_display_none ; ?>>
                                            <img class="widefat" src="<?php echo esc_url( $new_value ); ?>" alt="<?php esc_attr_e( 'Image preview', 'gutentor' ); ?>"  />
                                    </span><!-- .img-preview-wrap -->
                                    <input type="button" value="<?php esc_attr_e( 'Upload Image', 'gutentor' ); ?>" class="button gutentor-image-upload" data-title="<?php esc_attr_e( 'Select Image','gutentor'); ?>" data-button="<?php esc_attr_e( 'Select Image','gutentor'); ?>"/>
                                    <input type="button" value="<?php esc_attr_e( 'Remove Image', 'gutentor' ); ?>" class="button gutentor-image-remove" />
		...
		public function css_box( $values = array(),$key, $cssbox_fields  ) {
			...
			$link_text = isset( $attr['link_text'] )?$attr['link_text']:esc_html__('Link','gutentor');
			...
			foreach( $devices as $device_id => $device_details ){
				...
				foreach( $box_fields_attr as $field_id => $box_single_field ){
                    ...
                    <label>
                        ...
                        <input data-device="<?php echo esc_attr( $device_id )?>" data-cssbox-name="<?php echo esc_attr( $field_id )?>" data-default="<?php echo esc_attr( $default );?>" min="<?php echo esc_attr($min); ?>" max="<?php echo esc_attr($max); ?>" step="<?php echo esc_attr( $step );?>" type="number" class="gutentor-groupcssbox-field" value="<?php echo esc_attr( $value );?>">
                    </label>
					<?php
					echo "</li>";
				}
                    ...
                    <li>
                        <label>
                            ...
                            <span class="field-link">
                                <input data-device="<?php echo esc_attr( $device_id )?>" data-cssbox-name="cssbox_link" data-default="<?php echo esc_attr( $default );?>"  type="checkbox" class="gutentor-groupcssbox-field <?php echo $link_toggle_class;?>" value="<?php echo esc_attr( $value );?>" <?php checked(true, $cssbox_link,true)?>>
